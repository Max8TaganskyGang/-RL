"""
Упрощенный training pipeline для векторизованных сред
Адаптирован для работы с SyncVectorEnv и полноценной векторизацией
"""

import numpy as np
import torch
from typing import Dict, List, Optional
from collections import deque
import time
import os
import sys

# Добавляем корневую директорию в путь
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

try:
    import wandb
    WANDB_AVAILABLE = True
except ImportError:
    WANDB_AVAILABLE = False
    print("Warning: wandb not available. Install with: pip install wandb")

from dqn import DQNAgent


def train_dqn_vectorized(
    env,
    agent: DQNAgent,
    n_episodes: int = 500,
    max_steps_per_episode: int = 200,
    train_freq: int = 4,
    eval_freq: int = 50,
    eval_episodes: int = 10,
    save_dir: Optional[str] = None,
    use_wandb: bool = False,
    wandb_config: Optional[Dict] = None,
) -> Dict[str, List[float]]:
    """
    Обучение DQN агента на векторизованной среде.
    
    Args:
        env: Векторизованная среда (SyncVectorEnv или VectorizedGridWorldEnv)
        agent: DQN агент
        n_episodes: Количество эпизодов для обучения (суммарно по всем средам)
        max_steps_per_episode: Максимальное количество шагов в эпизоде
        train_freq: Частота обучения (каждые N шагов)
        eval_freq: Частота оценки (каждые N эпизодов)
        eval_episodes: Количество эпизодов для оценки
        save_dir: Директория для сохранения результатов
        use_wandb: Использовать ли Wandb для логирования
        wandb_config: Конфигурация для Wandb
    
    Returns:
        Словарь с метриками обучения (включая total_steps для сравнения)
    """
    metrics = {
        'episode_rewards': [],
        'episode_lengths': [],
        'losses': [],
        'eval_rewards': [],
        'eval_lengths': [],
        'total_steps': [],  # Общее количество шагов для каждого эпизода
    }
    
    episode_rewards_deque = deque(maxlen=100)
    episode_lengths_deque = deque(maxlen=100)
    
    # Определяем количество параллельных сред
    if hasattr(env, 'num_envs'):
        num_envs = env.num_envs
    elif hasattr(env, 'envs'):  # Для SyncVectorEnv
        num_envs = len(env.envs)
    else:
        num_envs = 1
    
    # Инициализация wandb
    if use_wandb and WANDB_AVAILABLE:
        if wandb_config is None:
            wandb_config = {}
        try:
            wandb.init(
                entity='sinitskii-mi',
                project=wandb_config.get('project', 'gridworld-vectorized'),
                name=wandb_config.get('name', 'dqn-vectorized'),
                config={
                    **wandb_config.get('config', {}),
                    'num_envs': num_envs,
                },
                reinit=True,
                mode='online'
            )
        except Exception as e:
            print(f"⚠️  Ошибка Wandb: {e}. Продолжаю обучение без логирования в Wandb.")
            use_wandb = False
    
    # Сброс среды
    obs, info = env.reset()
    # obs shape: (num_envs, obs_shape)
    
    total_steps_all = 0  # Общее количество шагов по всем средам
    episode_count = 0
    step_count = 0
    
    # Буферы для накопления данных по каждой среде
    episode_rewards_buf = np.zeros(num_envs)
    episode_lengths_buf = np.zeros(num_envs)
    episode_steps_buf = np.zeros(num_envs, dtype=int)  # Шаги для каждой среды
    
    start_time = time.time()
    
    print(f"🚀 Начало обучения DQN на векторизованной среде ({num_envs} параллельных сред)")
    print(f"📊 Общее количество эпизодов: {n_episodes}")
    print()
    
    while episode_count < n_episodes:
        # Выбор действий для всех сред
        actions = []
        for i in range(num_envs):
            # Для каждой среды выбираем действие
            if hasattr(agent, 'use_lstm') and agent.use_lstm:
                action = agent.select_action(obs[i], training=True, reset_hidden=False)
            else:
                action = agent.select_action(obs[i], training=True)
            actions.append(action)
        actions = np.array(actions)
        
        # Выполнение действий во всех средах
        next_obs, rewards, terminated, truncated, info = env.step(actions)
        done = terminated | truncated
        
        # Обновляем буферы для всех сред
        episode_rewards_buf += rewards
        episode_lengths_buf += 1
        episode_steps_buf += 1
        
        # Общее количество шагов (сумма по всем средам)
        total_steps_all += num_envs
        
        # Сохранение переходов в replay buffer для каждой среды
        for i in range(num_envs):
            agent.replay_buffer.push(
                obs[i],
                int(actions[i]),
                float(rewards[i]),
                next_obs[i],
                bool(done[i])
            )
            
            # Обучение на batch из replay buffer
            if len(agent.replay_buffer) > agent.batch_size and step_count % train_freq == 0:
                loss = agent.train_step()
                if loss is not None:
                    metrics['losses'].append(loss)
                    if use_wandb and WANDB_AVAILABLE:
                        wandb.log({
                            'loss': loss,
                            'total_steps': total_steps_all,
                        })
        
        # Обновление epsilon для агента
        if step_count % train_freq == 0:
            agent.update_epsilon()
        
        # Обработка завершенных эпизодов
        done_indices = np.where(done)[0]
        for i in done_indices:
            episode_count += 1
            episode_reward = float(episode_rewards_buf[i])
            episode_length = int(episode_lengths_buf[i])
            
            episode_rewards_deque.append(episode_reward)
            episode_lengths_deque.append(episode_length)
            
            metrics['episode_rewards'].append(episode_reward)
            metrics['episode_lengths'].append(episode_length)
            metrics['total_steps'].append(total_steps_all)  # Общее количество шагов на момент завершения эпизода
            
            if use_wandb and WANDB_AVAILABLE:
                wandb.log({
                    'episode': episode_count,
                    'episode_reward': episode_reward,
                    'episode_length': episode_length,
                    'total_steps': total_steps_all,
                    'epsilon': agent.epsilon,
                    'avg_reward_100': np.mean(episode_rewards_deque) if episode_rewards_deque else 0,
                    'avg_length_100': np.mean(episode_lengths_deque) if episode_lengths_deque else 0,
                })
            
            # Сброс буферов для завершенной среды
            episode_rewards_buf[i] = 0
            episode_lengths_buf[i] = 0
            episode_steps_buf[i] = 0
            
            if episode_count % 100 == 0:
                avg_reward = np.mean(episode_rewards_deque) if episode_rewards_deque else 0
                avg_length = np.mean(episode_lengths_deque) if episode_lengths_deque else 0
                elapsed = time.time() - start_time
                steps_per_sec = total_steps_all / elapsed if elapsed > 0 else 0
                print(f"Episode {episode_count}/{n_episodes} | "
                      f"Avg Reward: {avg_reward:.2f} | "
                      f"Avg Length: {avg_length:.2f} | "
                      f"Total Steps: {total_steps_all} | "
                      f"Steps/sec: {steps_per_sec:.1f} | "
                      f"Time: {elapsed:.1f}s")
        
        # Сброс завершенных сред
        if np.any(done):
            # Для SyncVectorEnv reset вызывается автоматически
            # Для VectorizedGridWorldEnv нужно сбросить вручную
            if hasattr(env, 'reset_single'):
                # Полноценная векторизация - сбрасываем только завершенные среды
                for i in done_indices:
                    obs[i], _ = env.reset_single(i)
                    episode_steps_buf[i] = 0  # Сбрасываем счетчик шагов
            else:
                # SyncVectorEnv - сброс автоматический, просто получаем новые obs
                # Reset уже произошел автоматически
                pass
        
        # Обновление наблюдений
        obs = next_obs
        step_count += 1
    
    elapsed_time = time.time() - start_time
    
    print(f"\n✅ Обучение завершено!")
    print(f"📊 Всего эпизодов: {episode_count}")
    print(f"📊 Всего шагов (сумма по всем средам): {total_steps_all}")
    print(f"⏱️  Время обучения: {elapsed_time:.2f}s")
    print(f"⚡ Шагов в секунду: {total_steps_all / elapsed_time:.2f}")
    
    # Сохранение метрик
    if save_dir:
        os.makedirs(save_dir, exist_ok=True)
        import pickle
        with open(os.path.join(save_dir, 'metrics.pkl'), 'wb') as f:
            pickle.dump(metrics, f)
    
    if use_wandb and WANDB_AVAILABLE:
        wandb.finish()
    
    return metrics


def evaluate_vectorized(env, agent, n_episodes: int = 10) -> tuple:
    """Оценка агента на векторизованной среде."""
    episode_rewards = []
    episode_lengths = []
    
    obs, info = env.reset()
    
    # Определяем количество сред
    if hasattr(env, 'num_envs'):
        num_envs = env.num_envs
    elif hasattr(env, 'envs'):
        num_envs = len(env.envs)
    else:
        num_envs = 1
    
    episode_rewards_buf = np.zeros(num_envs)
    episode_lengths_buf = np.zeros(num_envs)
    done_buf = np.zeros(num_envs, dtype=bool)
    episode_count = 0
    
    while episode_count < n_episodes:
        # Выбор действий
        actions = []
        for i in range(num_envs):
            if not done_buf[i]:
                if hasattr(agent, 'use_lstm') and agent.use_lstm:
                    action = agent.select_action(obs[i], training=False, reset_hidden=done_buf[i])
                else:
                    action = agent.select_action(obs[i], training=False)
                actions.append(action)
            else:
                actions.append(0)
        actions = np.array(actions)
        
        # Выполнение действий
        next_obs, rewards, terminated, truncated, info = env.step(actions)
        done = terminated | truncated
        
        # Обновление буферов
        active_mask = ~done_buf
        episode_rewards_buf[active_mask] += rewards[active_mask]
        episode_lengths_buf[active_mask] += 1
        
        # Обработка завершенных эпизодов
        newly_done = done & ~done_buf
        if np.any(newly_done):
            for i in np.where(newly_done)[0]:
                episode_rewards.append(float(episode_rewards_buf[i]))
                episode_lengths.append(int(episode_lengths_buf[i]))
                episode_rewards_buf[i] = 0
                episode_lengths_buf[i] = 0
                done_buf[i] = False
                episode_count += 1
        
        obs = next_obs
        done_buf = done
        
        # Сброс завершенных сред
        if np.any(done):
            if hasattr(env, 'reset_single'):
                for i in np.where(done)[0]:
                    obs[i], _ = env.reset_single(i)
                    done_buf[i] = False
    
    avg_reward = np.mean(episode_rewards) if episode_rewards else 0
    avg_length = np.mean(episode_lengths) if episode_lengths else 0
    
    return avg_reward, avg_length

